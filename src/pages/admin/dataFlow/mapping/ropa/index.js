    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <div className="bg-white min-h-screen rounded-lg p-5 my-3 mx-5">
        ...
        <div className="px-6 py-8 bg-[#2B245C] rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300">
          <h1 className="text-3xl font-bold text-cyan-50">ROPA</h1>
          <p className="mt-1 text-sm text-white">
            Pick an Activity (BPA) to generate a GDPR record.
          </p>
        </div>
          ...
          {loading ? (
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {[...Array(6)].map((_, i) => (
                ...
                >
                  <div className="h-5 w-36 animate-nonerounded bg-gray-100 mb-2"></div>
                  <div className="h-4 w-48 animate-nonerounded bg-gray-100"></div>
                </div>
              ))}
            </div>
          ) : error ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {items.map((it) => (
                <Link
                  key={it._id}
                  href={`./ropa/${it._id}`}
                  className="block rounded-xl border border-[#2B245C] bg-gray-50 p-4 shadow-sm hover:shadow-lg hover:bg-gray-100"
                >
