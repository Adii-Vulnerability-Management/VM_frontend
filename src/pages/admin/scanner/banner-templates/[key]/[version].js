// pages/admin/scanner/banner-templates/[key]/[version].jsx
  ...
  return (
    <div className="mb-4 last:mb-0">
      <button
        ...
        id={`section-${id}`}
        className="w-full flex justify-between items-center p-3 bg-white border border-gray-200 rounded-t-lg hover:bg-gray-50 focus:outline-none focus:ring"
      >
...
// Accordions with toggle under Pref modal
...
function ManagePreferencesCenter({
  ...
  return (
    <div className="px-4 py-3">
      ...
      {(form.showVendorList || form.showCookieList) && (
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        >
          {form.prefBackIcon ? (
            <img
              src={
                typeof form.prefBackIcon === "string"
                  ? form.prefBackIcon
                  : URL.createObjectURL(form.prefBackIcon)
              }
              alt="Back"
              title="Back"
              style={{
...
export default function TemplateForm() {
  ...
  const { key, version } = router.query;
  const isNew = !key || !version;

  // Tracks if the form has unsaved changes
  const [isDirty, setIsDirty] = useState(false);

  // Centralized defaults for banner + preference center + categories
  ...
  // Load template from API if we are editing an existing one
  useEffect(() => {
  ...
  async function fetchTemplate() {
    ...
    try {
      const res = await CustomAxios.get(
        `${baseurl}/${initURL}/cmp/templates/${key}/${version}`,
      );
      const d = res.data.defaults;
      // Normalize cookieCategories in case backend returns an object instead of array
      const normalizedCats = Array.isArray(d.cookieCategories)
        ? d.cookieCategories
        : d.cookieCategories
          ? Object.values(d.cookieCategories)
          : [];
      setForm((f) => ({
        ...f,
        ...d,
        layout: { ...d.layout, type: d.layout?.type || "bar" },
        cookieCategories: normalizedCats,
      }));
      initialFormRef.current = { ...form, ...d };
    } finally {
      setLoading(false);
    }
  }

  // Warn user before leaving / refreshing the page if there are unsaved changes
  useEffect(() => {
    const handle = (e) => {
      if (!isDirty) return;
      e.preventDefault();
  ...
  // Warn user when changing Next.js routes with unsaved changes
  useEffect(() => {
    const start = (url) => {
  ...
  // Helper to mark the form as dirty
  function markDirty() {
    setIsDirty(true);
  }

  // Unified handler for inputs (top level + deeply nested)
  function onChange(e) {
    markDirty();
    ...
    // 4) Update simple top-level field
    setForm((f) => ({ ...f, [name]: type === "checkbox" ? checked : value }));
  }

  // Append a new blank cookie category
  ...
  // Remove a cookie category by index
  ...
  // Initialize category + item states when cookieCategories change
  useEffect(() => {
    const catStates = {};
    const itemStates = {
      vendors: {},
  ...
  // Toggle a specific vendor/cookie item ON/OFF
  ...
  // Save the template to the backend (create or update)
  async function onSubmit(e) {
    e.preventDefault();
    setLoading(true);
    const payload = { key: form.key, version: form.version, defaults: form };
    try {
      if (isNew) {
        await CustomAxios.post(`${baseurl}/${initURL}/cmp/templates`, payload);
      } else {
      ...
      setIsDirty(false);
      router.push("/admin/scanner/banner-templates");
    } catch {
      alert("Save error");
    } finally {
      setLoading(false);
  ...
  // Destructure common values from form for readability
  const {
  ...
  // Memoized banner inline style based on selected layout and options
  const bannerStyle = useMemo(() => {
    ...
    if (layoutType === "bar") {
      // full-width top/bottom OR full-height left/right
      ...
      // corner toasts: top-left, top-right, bottom-left, bottom-right
      const [vert, hor] = (position || "bottom-right").split("-");
      base[vert] = "1rem";
      base[hor] = "1rem";
      base.width = "260px";
    } else if (layoutType === "floating") {
      // floating supports center, top-center, bottom-center, and corners
  ...
  // Compute allowed positions for the current layout type
  ...
  // If current position isn't valid for the selected type, snap to the first valid
  useEffect(() => {
  ...
  }, [layoutPositionOptions]); // re-check whenever type/options change

  // Buttons in different layouts and orientation
  const isCompactLayout =
    layoutType === "toast" ||
    layoutType === "floating" ||
    (layoutType === "bar" && (position === "left" || position === "right"));

  // grow buttons only in compact, non-stacked, horizontal rows
  const growButtonsRow =
    isCompactLayout && !form.bannerButtonStyles.stackButtons;

  // Convert logical align values into flexbox justifyContent
  const toJustify = (a) =>
  ...
  // Helper to style the wrapper around banner buttons (stacked vs inline)
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <aside className="lg:col-span-1 max-h-[calc(100vh-4rem)] overflow-auto bg-gray-50 rounded-lg shadow-lg">
          <form onSubmit={onSubmit} className="space-y-6 mb-8 p-6">
            {/* Header with template key/version */}
            <h1 className="text-2xl font-bold text-[#2B245C]">
              {isNew ? "New Template" : `${form.key}@${form.version}`}
            </h1>
            {/* Basic key + version inputs */}
            ...
            <div className="mt-2">
              <label className="text-xl text-[#2B245C] font-semibold">
                Banner Section
              </label>
            </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <div>
                  ...
                  {form.prefHeaderLogo && (
                    <img
                      src={
                        form.prefHeaderLogo instanceof File
                          ? URL.createObjectURL(form.prefHeaderLogo)
                          : form.prefHeaderLogo
                      }
              ...
              <div className="grid grid-cols-1 gap-3">
                <div>
                  ...
                  {form.prefFooterLogo && (
                    <img
                      src={
                        form.prefFooterLogo instanceof File
                          ? URL.createObjectURL(form.prefFooterLogo)
                          : form.prefFooterLogo
                      }
            ...
            {/* Add image to show "back" button in Vendor/Cookie list center */}

            {/* <Section id="list-icon" title="List Section Icon">
              <div>
                <label className="block text-sm">Back Icon</label>
                <input type="file" name="prefBackIcon" onChange={onChange} />
                {form.prefBackIcon && (
                  <img
                    src={
                      typeof form.prefBackIcon === "string"
            ...
            <div className="flex space-x-4">
              <button
                type="submit"
                className="px-6 py-2 bg-[#2B245C] text-white rounded-lg hover:bg-opacity-90 transition"
              >
                {isNew ? "Create" : "Save"}
              </button>
              <button
                type="button"
                onClick={() => router.back()}
                className="px-6 py-2 bg-white border border-[#2B245C] text-[#2B245C] rounded-lg hover:bg-gray-200 transition"
              >
                Cancel
              </button>
            </div>
              ...
              <div>
                ...
                >
                  ...
                  {form.prefFooterLogo && (
                    <img
                      src={
                        typeof form.prefFooterLogo === "string"
                          ? form.prefFooterLogo
                          : URL.createObjectURL(form.prefFooterLogo)
                      }
                      alt="Footer logo"
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                {form.prefHeaderLogo && (
                  <img
                    src={
                      typeof form.prefHeaderLogo === "string"
                        ? form.prefHeaderLogo
                        : URL.createObjectURL(form.prefHeaderLogo)
                    }
