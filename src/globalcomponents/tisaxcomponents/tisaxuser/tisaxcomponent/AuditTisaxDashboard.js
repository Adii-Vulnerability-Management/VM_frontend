...
function AuditTisaxDashboard() {
    ...
    useEffect(() => {
        if (id && vda_type && assessment_level && vda_version) {
            CustomAxios.get(`${baseurl}/${initURL}/assign-tisax-task/employee-assigned-tasks`)
                .then((response) => {
                    const rawData = response.data.find((loc) => loc._id == rootId) || {};

                    // Define valid categories based on available data
                    const validCategories = [
                        rawData.informationSecurityControls?.length ? { name: "Information Security", categories: [] } : null,
                        rawData.dataProtectionControls?.length ? { name: "Data Protection", categories: [] } : null,
                        rawData.prototypeProtectionControls?.length ? { name: "Prototype Protection", categories: [] } : null,
                    ].filter(Boolean); // Remove null values

                    if (validCategories.length === 0) return; // If no categories, do nothing
                    ...
                    const processControls = (controls, categoryName) => {
                        ...
                        validCategories.find((cat) => cat.name === categoryName).categories = Object.values(categoryData);
                    };

                    // Process each control category if present
                    if (rawData.informationSecurityControls?.length) {
                        processControls(rawData.informationSecurityControls, "Information Security");
                    }
                    if (rawData.dataProtectionControls?.length) {
                        processControls(rawData.dataProtectionControls, "Data Protection");
                    }
                    if (rawData.prototypeProtectionControls?.length) {
                        processControls(rawData.prototypeProtectionControls, "Prototype Protection");
                    }
