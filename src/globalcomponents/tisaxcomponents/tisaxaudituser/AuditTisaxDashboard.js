    ...
    if (id && vda_type && assessment_level && vda_version) {
      CustomAxios.get(
        ...
        .then((response) => {
          ...
          // Define only categories that have valid data
          ...
          if (validCategories.length === 0) return; // If no valid categories exist, do nothing
          ...
          const processControls = (controls, categoryName) => {
            ...
            controls.forEach((control) => {
              const controlKey =
                ...
                control["Parent Control question"];
              const isaNewKey =
                control["Root ISA New"] || control["Parent ISA New"];
          ...
          // Dynamically map and sort sections by `rootISANew`, filtering out empty ones
