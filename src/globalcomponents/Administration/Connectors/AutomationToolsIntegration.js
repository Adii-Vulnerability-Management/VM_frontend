...
const AutomationToolsIntegration = () => {
  ...
  const handleSyncTool = (id) => {
    setTools(
      tools.map((tool) =>
        tool.id === id ? { ...tool, workflows: tool.workflows + 1 } : tool,
      ),
    );
  };
  ...
  return (
    <div className="p-2 min-h-screen">
      ...
      <p className="text-gray-600 text-sm mb-6">
        Manage automation tools like Tines, Torq, and Tray.io to automate
        workflows for GRC processes.
      </p>
      ...
      <div className="mb-5 rounded-2xl border border-[#2B245C] bg-white p-6 shadow-lg hover:shadow-2xl transition-shadow duration-300">
        <h3 className="text-2xl font-bold text-[#2B245C] mb-4">Add New Tool</h3>
...
export default AutomationToolsIntegration;
