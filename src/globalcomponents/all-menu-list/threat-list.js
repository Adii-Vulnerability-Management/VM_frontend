...
export default function Threats() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setItems(res?.data?.threatsData || []);
    } catch (e) {
      toast.error(e?.response?.data?.message || "Failed to load threats");
    } finally {
      setLoading(false);
    }
  };
  ...
  const addItem = async (e) => {
    ...
    try {
      await createThreat(newName.trim());
  ...
  const saveEdit = async (it) => {
    ...
    try {
      await createThreat(next);
      await deleteThreat(it._id);
      toast.success("Threat renamed");
      cancelEdit();
      fetchData();
    } catch (e) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-semibold">Threats List</h3>
