...
const Assign = () => {
  ...
  const [allreadyAssignedLocationIds, setallreadyAssignedLocationIds] =
    useState([]);
  ...
  const handleSelectLocation = (location) => {
    const isAlreadyAssigned = allreadyAssignedLocationIds.includes(
      location._id
    );
    if (isAlreadyAssigned) {
      toast.warning("This location is already assigned.");
      return;
    }
  ...
  return (
    <div>
      ...
      <Dialog
        ...
        title={selectedLocation ? "Selected Location" : "Search Location"}
        footer={
          selectedLocation && (
            <>
              <button
              ...
              <button
                ...
                className="px-5 py-3 bg-[#2B245C] text-white rounded-xl hover:bg-[#2B245C] transition text-lg"
              >
                Submit
              </button>
            </>
          ...
          <>
            ...
            <div className="flex items-center space-x-3 w-full mb-6">
              <input
                ...
                className="border border-gray-300 rounded-xl p-3 w-full focus:ring-2 focus:ring-[#2B245C] text-lg"
              />
              <button
                onClick={handleSearch}
                className="bg-[#2B245C] hover:bg-[#2B245C] text-white px-5 py-3 rounded-xl flex items-center text-lg transition"
              >
            ...
            <div className="space-y-4 max-h-60 overflow-y-auto">
              {locations.length > 0 ? (
                locations
                  .filter((l) => l.vda_version === "6.0.3")
                  .map((loc) => (
                    <div
                      key={loc._id}
                      onClick={() => handleSelectLocation(loc)}
                      className={`p-4 border border-gray-200 rounded-xl cursor-pointer transition shadow-sm hover:bg-gray-100 ${
                        allreadyAssignedLocationIds.includes(loc._id)
