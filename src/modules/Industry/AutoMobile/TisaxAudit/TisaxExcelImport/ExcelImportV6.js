...
} from "@/store/SliceComponent/DataSlice";
...
const ExcelImportV6 = ({assessmentLevel, locationId , vdaType}) => {
  const [jsonData, setJsonData] = useState({});
  const [tableData, setTableData] = useState([]);
  const dispatch = useDispatch();
  const assessmentlevel = assessmentLevel;
  const id = locationId;
  const vda_type = vdaType;
  useEffect(() => {
    ...
    const rawProto = convertedData["prototype protection"] || [];
    const flatProto = rawProto.flatMap((group) => group.Items || []);
    const convertedDataNewPPD = flatProto.map(cleanKeysAndValues);

    if (
      Array.isArray(convertedData["prototype protection"]) &&
      convertedData["prototype protection"].length > 0
    ) {
      convertedData["prototype protection"][0].Items = convertedDataNewPPD;
    }
    console.log("Converted Data New PPD ExcelImportV6.0.3", convertedData);
    ...
    reader.onload = (e) => {
      ...
      workbook.SheetNames.forEach((sheetName) => {
        ...
        if (
          ...
          if (
            compareTwoStrings(trimmedSheetName, "information security") > 0.6
          ) {
            ...
            const isaNewColumnIndex =
              jsonData[informationSecurityHeaderIndexNumber].indexOf(
                "Control number"
              );

            if (isaNewColumnIndex !== -1) {
              jsonData[informationSecurityHeaderIndexNumber][
                isaNewColumnIndex
              ] = "ISA New";
            }
            ...
            const isaNewColumnIndex =
              jsonData[dataProtectionHeaderIndexNumber].indexOf(
                "Control number"
              );

            if (isaNewColumnIndex !== -1) {
              jsonData[dataProtectionHeaderIndexNumber][isaNewColumnIndex] =
                "ISA New";
            }
            ...
            const isaNewColumnIndex =
              jsonData[prototypeProtectionHeaderIndexNumber].indexOf(
                "Control number"
              );

            if (isaNewColumnIndex !== -1) {
              jsonData[prototypeProtectionHeaderIndexNumber][
                isaNewColumnIndex
              ] = "ISA New";
            }
    ...
    <div>
      ...
      {tableData.length > 0 && (
        <Link
          href={
            id
              ? `/industry/automobile/importtisaxlocationexcel?id=${id}&vda_type=${vda_type}&assessment_level=${assessmentlevel}&vda_version=6.0.3`
              : `/industry/automobile/importtisaxlocationexcel?vda_type=${vda_type}&assessment_level=${assessmentlevel}&vda_version=6.0.3`
          }
        >
          <button
