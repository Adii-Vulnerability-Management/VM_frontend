...
export default function ThirdPartyAuditReportUploadModal({
  isOpen,
  onClose,
  thirdPartyId,
  onSuccess,
  ...
  const handleUpload = async (e) => {
    ...
    if (!thirdPartyId) {
      toast.error("Third-party ID missing.");
      return;
    }

    if (!file) {
      toast.error("Attach a report file before uploading.");
      return;
    }
    ...
    try {
      await uploadAuditReport(thirdPartyId, {
        file,
        docType,
      ...
      onSuccess?.("Report uploaded successfully.");
      resetForm();
      onClose();

      (async () => {
        try {
          await checkS3NewFiles(thirdPartyId);
          await syncThirdPartyEvidence(thirdPartyId);
          await ingestAuditEvidence(thirdPartyId);
          await getAuditEvidenceStatus(thirdPartyId);
          await cleanupOrphanedChunks(thirdPartyId);
        } catch (syncErr) {
      ...
      toast.error(
        err?.response?.status === 413
          ? "Server rejected the upload (413: Payload Too Large)."
