...
import VendorAuditReportUploadModal from "./VendorAuditReportUploadModal";
...
export default function VendorAuditReportUpload({ vendorId }) {
  ...
  const safeVendorId =
    normalizedVendorId &&
    normalizedVendorId.toLowerCase() !== "null" &&
    normalizedVendorId.toLowerCase() !== "undefined"
      ? normalizedVendorId
      : "";
  ...
  const loadDocs = async () => {
    ...
    setError("");
    try {
      const data = await getAuditReports(safeVendorId);
  ...
  const handleDelete = async (docId, hardDelete) => {
    ...
    setSuccess("");
    try {
      await deleteAuditReport(safeVendorId, docId, hardDelete);
      setSuccess("Report removed.");
      await loadDocs();
      setTimeout(() => setSuccess(""), 5000); // Clear success message after 5 seconds

      if (hardDelete) {
        // Run evidence processing in background so UI finishes faster
        (async () => {
          try {
            console.log(`[Table] Background sync starting for ${safeVendorId}`);
            await checkS3DeletedFiles(safeVendorId);
            await syncVendorEvidence(safeVendorId);
            await cleanupOrphanedChunks(safeVendorId);
            console.log(
              `[Table] Background sync completed for ${safeVendorId}`,
            );
          } catch (syncErr) {
            console.error(
              "[Table] Background evidence processing failed:",
              syncErr,
            );
          }
        })();
