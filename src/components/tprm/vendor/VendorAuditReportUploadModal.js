...
} from "../../../services/tprm/vendor/auditReportApi";
...
export default function VendorAuditReportUploadModal({
    isOpen,
    onClose,
    vendorId,
    onSuccess
    ...
    const handleUpload = async (e) => {
        ...
        try {
            console.log(`[Modal] Triggering uploadAuditReport for vendor: ${vendorId}`);
            await uploadAuditReport(vendorId, {
                file,
                docType,
            ...
            // Background evidence processing (don't await to keep UI responsive)
            (async () => {
                try {
                    console.log(`[Modal] Background sync starting for ${vendorId}`);
                    await checkS3NewFiles(vendorId);
                    await syncVendorEvidence(vendorId);
                    await cleanupOrphanedChunks(vendorId);
                    console.log(`[Modal] Background sync completed for ${vendorId}`);
                } catch (syncErr) {
                    console.error("[Modal] Background evidence processing failed:", syncErr);
                }
            })();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="relative w-full max-w-2xl rounded-[1.5rem] bg-white shadow-2xl transition-all">
                ...
                <div className="flex items-center justify-between px-8 pt-8 pb-4">
                    <h3 className="text-2xl font-bold text-slate-900">Upload New Report</h3>
                    <button
                        onClick={() => {
                            resetForm();
                            onClose();
                ...
                <form className="px-8 pb-8 space-y-6" onSubmit={handleUpload}>
                    <div className="space-y-2">
                        ...
                        <label className="flex cursor-pointer items-center justify-between rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3.5 text-sm text-slate-700 transition hover:border-slate-400">
                            <div className="flex flex-col">
                                <span className="font-bold text-slate-900">{file ? file.name : "Drop a file or browse"}</span>
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    if (selected && isAllowedFile(selected) && selected.size <= MAX_UPLOAD_BYTES) {
                                        setFile(selected);
                                        setError("");
                                    } else if (selected) {
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            <label className="block text-sm font-semibold text-slate-700">Issued on</label>
                            <input
                                type="date"
                                value={issuedAt}
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        <div className="space-y-2">
                            <label className="block text-sm font-semibold text-slate-700">Period start</label>
                            <input
                                type="date"
                        ...
                        <div className="space-y-2">
                            <label className="block text-sm font-semibold text-slate-700">Period end</label>
                            <input
                                type="date"
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-semibold text-slate-700">Expires on (optional)</label>
                        <input
                            type="date"
                            value={expiresAt}
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-semibold text-slate-700">Notes / Context</label>
                        <textarea
                            value={notes}
